  ...
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div class="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
        ...
        <div class="text-2xl font-bold text-white font-mono">{{ skills.length }} Yetenek</div>
      </div>
      <div class="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
      ...
      <div class="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-1">
        ...
        <div class="text-sm font-semibold text-emerald-400 mt-1 flex items-center gap-1.5 font-mono">
        ...
        <form @submit.prevent="submitCreateSkill" class="space-y-4">
          <div>
            ...
            <input
              ...
              required
              class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-xs text-white focus:outline-none focus:border-indigo-500 transition"
            />
          </div>
          ...
          <div class="pt-3 border-t border-slate-800 flex justify-end gap-3">
            ...
            <button
              ...
              class="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-xl text-xs font-semibold transition shadow-lg shadow-indigo-600/20"
            >
...
const fetchSkills = async () => {
  ...
  try {
    ...
    toast.error(err?.data?.statusMessage || 'İlerleme verileri yüklenirken bir hata oluştu.')
  } finally {
    loading.value = false
  }
}
...
const handleUpdateProgress = async (id: number, newPercentage: number) => {
  ...
  if (target) target.percentage = clamped

  try {
    const res = await $fetch<{ success: boolean; skill: SkillItem }>('/api/tracker/progress', {
      method: 'POST',
      body: { id, percentage: clamped },
    })

    if (res?.skill) {
      const idx = skills.value.findIndex(s => s.id === id)
      if (idx !== -1) skills.value[idx] = res.skill
      toast.success(`${res.skill.skillName} %${res.skill.percentage} olarak güncellendi.`)
    }
  } catch (err: any) {
    skills.value = previousSkills
    toast.error(err?.data?.statusMessage || 'İlerleme güncellenirken hata oluştu.')
  }
}
...
const handleIncrementProgress = async (id: number, delta: number) => {
  const previousSkills = JSON.parse(JSON.stringify(skills.value)) as SkillItem[]
  const target = skills.value.find(s => s.id === id)
  if (target) target.percentage = Math.min(100, Math.max(0, target.percentage + delta))

  try {
    const res = await $fetch<{ success: boolean; skill: SkillItem }>('/api/tracker/progress', {
      method: 'POST',
      body: { id, increment: delta },
    })

    if (res?.skill) {
      const idx = skills.value.findIndex(s => s.id === id)
      if (idx !== -1) skills.value[idx] = res.skill
      toast.success(`${res.skill.skillName} +%${delta} artırıldı.`)
    }
  } catch (err: any) {
    skills.value = previousSkills
    toast.error(err?.data?.statusMessage || 'İlerleme güncellenirken hata oluştu.')
...
const handleDeleteSkill = async (id: number) => {
  const ok = await confirm.ask({
    title: 'Yeteneği Sil',
    message: 'Bu yetenek başlığını silmek istediğinizden emin misiniz?',
  ...
  try {
    ...
    skills.value = previousSkills
    toast.error(err?.data?.statusMessage || 'Yetenek silinirken hata oluştu.')
  }
}
...
const submitCreateSkill = async () => {
  ...
  try {
    ...
    toast.error(err?.data?.statusMessage || 'Yetenek eklenirken bir hata oluştu.')
  } finally {
